...
<Note>
  ...
  <Tab title="Multi-Extraction Mode">

### Request

Apply one schema across multiple extractions combined. This is useful when related data spans multiple files (e.g., a set of invoices, multi-file reports).

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `extraction_ids` | uuid[] | Yes | List of saved extraction IDs to combine. Cannot be used with `extraction_id` or `split_id`. |
...
schema_result = client.schema(
    extraction_ids=["extraction-uuid-1", "extraction-uuid-2", "extraction-uuid-3"],
    schema_config={
        "schema": {
            "type": "object",
            "properties": {
        ...
        "schema_prompt": "Extract combined financial data across all documents"
    }
)
...
<Note>
  In multi-extraction mode, citation bounding box IDs are prefixed with the extraction ID (e.g., `abc123-txt-1`) to disambiguate across files.
</Note>
