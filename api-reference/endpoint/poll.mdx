...
for ((attempt=1; attempt<=MAX_ATTEMPTS; attempt++)); do
    response=$(curl -s "https://dev.api.runpulse.com/job/${JOB_ID}" \
        -H "x-api-key: ${API_KEY}")
...
```bash curl
# Check job status
curl https://dev.api.runpulse.com/job/abc123-def456-ghi789 \
...
```bash curl
# Step 1: Submit async extraction
JOB_RESPONSE=$(curl -s -X POST https://dev.api.runpulse.com/extract_async \
  -H "x-api-key: YOUR_API_KEY" \
...
DELAY=1
while true; do
    STATUS_RESPONSE=$(curl -s "https://dev.api.runpulse.com/job/${JOB_ID}" \
        -H "x-api-key: YOUR_API_KEY")
