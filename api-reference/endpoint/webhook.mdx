...
```bash curl
curl -X POST https://dev.api.runpulse.com/webhook \
...
app.post('/webhook', async (req, res) => {
    ...
    if (event.type === 'job.completed') {
        ...
        if (jobResult.result) {
            console.log(`Content: ${jobResult.result.content?.slice(0, 100)}...`);
            
            if (jobResult.result.structuredOutput) {
                console.log(`Structured data:`, jobResult.result.structuredOutput);
            }
        }
    }
...
async function submitJob(): Promise<string> {
    ...
    console.log(`Job submitted: ${response.jobId}`);
