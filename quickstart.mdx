...
def extract_document_from_file(file_path, schema=None, pages=None):
    ...
    Args:
        ...
        pages: Optional page range, 1-indexed (e.g., "1-5" for first 5 pages)
    ...
    with open(file_path, 'rb') as f:
        ...
        if schema:
            data['structuredOutput'] = json.dumps({'schema': schema})
        if pages:
            data['pages'] = pages
...
Both `/extract` and `/extract_async` accept file uploads directly via multipart/form-data. Use the `file` parameter to upload your document:

```python direct_upload.py
def extract_with_file_upload(file_path, schema=None, pages=None):
    """
    Upload and extract a file in a single request.
    This is the simplest approach for most use cases.
    
    Args:
        file_path: Path to the local file
        schema: Optional JSON schema for structured extraction
        pages: Optional page range, 1-indexed (e.g., "1-5" or "1,3,5")
    ...
    with open(file_path, 'rb') as f:
        ...
        if schema:
            data['structuredOutput'] = json.dumps({
                'schema': schema
            })
        if pages:
            data['pages'] = pages
    ...
    if response.status_code == 200:
        ...
        if result.get('is_url'):
            print(f"Large document detected. Fetching from: {result['url']}")
            return requests.get(result['url']).json()
        return result
    else:
...
# Example usage
result = extract_with_file_upload("invoice.pdf", pages="1-5")
print(result['markdown'])
```

<Tip>
Use `file` for direct uploads or `fileUrl` when you have a public/presigned URL. Both work with `/extract` and `/extract_async`.
</Tip>
...
class PulseAPIClient:
    ...
    def extract(self, 
        ...
        Args:
            ...
            pages: Page range, 1-indexed (e.g., "1-5" for first 5 pages)
    ...
    def _extract_from_url(self, endpoint: str, file_url: str,
        ...
        if schema:
            data['structuredOutput'] = {'schema': schema}
        if pages:
            data['pages'] = pages
        if extract_figure:
            data['extractFigure'] = extract_figure
